  ...
  switch (action.type) {
    case types.REMOVE_TABLE: {
      ...
      const newState = state.filter(
        relation =>
          relation.source.tableId !== action.id &&
    ...
    case types.REMOVE_COLUMN: {
      ...
      const newState = state.filter(
        relation =>
          relation.source.columnId !== columnId &&
    ...
    case types.SAVE_FOREIGN_KEY_RELATION:
      console.log("Saving Foreing Key");
          ...
          $push: [
            {
              ...
              relationship: action.columnData.foreignKey.relationship
            }
          ]
        });
      }
    ...
    case types.UPDATE_FOREIGN_KEY_RELATION: {
      ...
      if (foreignKey.on.id) {
        ...
        const newState = state.map(relation => {
          if (relation.source.columnId === action.columnData.id) {
            ...
            matched = true;
            console.log(action.columnData.foreignKey.relationship);
            return {
              source: {
                columnId: action.columnData.id,
                tableId: action.tableId
      ...
      const newState = state.filter(
        relation => relation.source.columnId !== action.columnData.id
      );
